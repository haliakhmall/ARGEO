...
public class mcqManager : MonoBehaviour {
	...
	public Text questionfield, showanswerfield,questionNumberTf,
	timeText, option1Text, option2Text, option3Text, option4Text;
	...
	void retrieveQuestionfromDatabase(){
		...
		reference.Child("MCQ").Child(categoryNameInDatabse).Child(randomIndex.ToString()).Child("Question")
			.GetValueAsync().ContinueWith(task => {
				if (task.IsFaulted) {
					...
					questionfield.text = snapshot.Value.ToString();
				}
			});

		reference.Child("MCQ").Child(categoryNameInDatabse).Child(randomIndex.ToString()).ValueChanged += HandleValueChanged;
	...
	IEnumerator nextQuestion(){
		...
		// Make the buttons not clickable again
		option1Btn.interactable = false;
		option2Btn.interactable = false;
		option3Btn.interactable = false;
		option4Btn.interactable = false;
